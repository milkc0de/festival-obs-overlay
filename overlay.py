...
# Windows-specific imports for background input
...
class OverlaySettings:
    ...
    def load(self):
        # Default layout: 5 lanes + 1 OD (Overdrive)
        default_config = {
            "lanes": [
            ...
            "bg_color": [255, 0, 255], # OBS Chroma Key (Magenta)
            "off_color": [20, 20, 20],
...
# --- Windows API Helpers for Background Input ---
def is_key_pressed_win32(key_name):
    """Checks keyboard state globally on Windows (even if window is not focused)."""
...
def open_settings(settings):
    ...
    tk.Button(root, text="SAVE & LAUNCH", command=on_launch, bg=ACCENT, fg="black", font=("Arial", 11, "bold")).pack(pady=20)
    root.mainloop()

# --- Main Application ---
def run_overlay(settings):
    # Enable background events for controllers
    ...
    while True:
        ...
        for i, lane in enumerate(data["lanes"]):
            ...
            if joystick:
                i_type, i_id = lane.get("input_type", "button"), lane.get("input_id", 0)
                try:
                    if i_type == "button":
